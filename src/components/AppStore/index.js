...
class AppStore extends Component {
  ...
  getMatchedItems = event => {
    this.setState({appTextInput: event.target.value})
  }

  render() {
    const {tabCategory, appTextInput} = this.state
    const tabItemsList = appsList.filter(
      eachApp => eachApp.category === tabCategory,
    )
    const filteredAppList = tabItemsList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(appTextInput.toLowerCase()),
    )
      ...
      <div className="app-store-container">
        ...
        <ul className="tab-items-list">
          {tabsList.map(eachTab => (
            <TabItem
              displayAppItems={this.displayAppItems}
              tabDetails={eachTab}
              key={eachTab.tabId}
              isActive={eachTab.tabId === tabCategory}
            />
          ))}
        </ul>
